...
| `web/` | React/Vite UI, three routes, no router dependency (a plain pathname switch): `/` is the **customer onboarding page**, opened on the customer's own device, calling the contract directly (`deposit`, `registerFinger`, `setAllowance`, `withdrawToWallet`) — `terminal/` is never involved in onboarding. `/pos` is the **merchant POS screen**, run in Chromium kiosk mode on the terminal machine, driven by state from `terminal/` over WebSocket. `/display` is a browser stand-in for the physical OLED (same WebSocket feed, receive-only) — lets the two-screen flow be tested with no ESP32 attached. |
